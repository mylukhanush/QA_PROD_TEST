...
from app.utils.kpi_steps import KPI_STEPS, KPI_DOWNLOAD_STEPS
from app.utils.kpi_excel import generate_kpi_excel, generate_download_kpi_excel
...
_active_download_kpi_runs = {}
_active_download_kpi_runs_lock = threading.Lock()
...
def _wait_and_capture_kpi_data(page, start_time_ref, timeout_sec=45.0, cancel_check_fn=None):
    ...
    # 1. Wait up to 5s (polling every 200ms) to see if a visible paginator exists inside an active content container.
    # If the page already has loaded its data (rows or empty state) and no spinner is active, we break early to avoid delays.
    has_paginator = False
    for _ in range(25):
        if cancel_check_fn:
            cancel_check_fn()
        ...
        # Check if the page is already loaded without a paginator
        spinner_visible = False
        try:
            for spin_sel in ['.loading', '.loader', '[class*="spinner"]', 'mat-progress-bar']:
                loc = page.locator(spin_sel)
        ...
        if not spinner_visible:
            ...
            rows_visible = False
            for prefix in containers:
                loc = page.locator(f"{prefix}table tbody tr:visible, {prefix}mat-row:visible, {prefix}.mat-row:visible, {prefix}.mat-mdc-row:visible, {prefix}tbody tr[role='row']:visible, {prefix}tr[role='row']:visible:not(.mat-header-row):not(.mat-mdc-header-row):not([class*='header'])")
                if loc.count() > 0:
                    first_row_text = loc.first.text_content().lower()
                    if "loading" not in first_row_text:
                        rows_visible = True
                        break
            if rows_visible:
                # Wait up to 300ms (polling every 100ms) to check if a paginator element appears.
                # If so, set has_paginator = True so we prioritize waiting for it to load.
                for _ in range(3):
    ...
    while time.time() - start_wait < timeout_sec:
        ...
        if paginator:
            ...
            # Check if paginator has a valid non-zero count
            ...
            if m:
                ...
                if val > 0:
                    rec_val = val
                    break
                # "of 0" — don't break immediately; data might still be loading.
                # Only accept 0 after 10s timeout.
            # If paginator exists but does not yet contain a valid count, hold for it up to 10s
...
def _extract_visible_paginator_total(page):
    """Return the largest visible Angular paginator total, if one has loaded."""
    max_total = None
    for base_sel in ['.mat-paginator-range-label:visible', '.mat-mdc-paginator-range-label:visible']:
        try:
            loc = page.locator(base_sel)
            for idx in range(min(loc.count(), 10)):
                txt = loc.nth(idx).text_content() or ""
                match = re.search(r"\bof\s+([\d,]+)\b", txt, re.IGNORECASE)
                if match:
...
def _extract_command_center_list_api_total(url, body):
    """Extract totals only from APIs that back Command Centre list data."""
    url_lower = (url or "").lower()

    if "ccdashboard/ccmetadata" in url_lower and isinstance(body, dict):
        data = body.get("data")
        if isinstance(data, list):
            return len(data)

    if "tracker/v1/assetdetails" in url_lower and isinstance(body, dict):
        total = 0
        for key in ("assetList", "underMaintenanceList"):
            val = body.get(key)
...
def _click_command_center_list_view(page, list_btn=None):
    """Click the current Command Centre List View toggle even if Angular refreshes it."""
    for _ in range(12):
        try:
            clicked = page.evaluate("""() => {
                const el = document.querySelector('#listView');
                if (!el) return false;
                el.click();
                return true;
            }""")
    ...
    try:
        page.locator("#listView").first.click(timeout=3000, force=True)
        return True
    except Exception:
        pass
    ...
    if list_btn:
        try:
            list_btn.click(timeout=3000, force=True)
            return True
        except Exception:
            pass
...
def _wait_for_command_center_list_data(page, start_time_ref, api_response_data, timeout_sec=45.0, cancel_check_fn=None):
    """Wait until Command Centre list data has a real API or paginator total."""
    best_total = None
    best_time = None
    start_wait = time.time()
    ...
    while time.time() - start_wait < timeout_sec:
        ...
        api_total = api_response_data.get("rec_count")
        if isinstance(api_total, int) and api_total > 0:
            best_total = api_total if best_total is None else max(best_total, api_total)
            best_time = api_response_data.get("load_time") or time.time()

        paginator_total = _extract_visible_paginator_total(page)
        if paginator_total:
            best_total = paginator_total if best_total is None else max(best_total, paginator_total)
            best_time = best_time or time.time()
            break

        if best_total:
            # Give the Angular paginator a short chance to render after the API arrives.
            page.wait_for_timeout(1000)
            paginator_total = _extract_visible_paginator_total(page)
            if paginator_total:
                best_total = max(best_total, paginator_total)
            break
    ...
    if best_total:
        try:
            page.wait_for_selector(
                'table tbody tr:visible, mat-row:visible, .mat-row:visible, .mat-mdc-row:visible',
                timeout=10000
            )
        except Exception:
            pass
        elapsed = f"{(best_time or time.time()) - start_time_ref:.2f}"
        return elapsed, str(best_total)

    return _wait_and_capture_kpi_data(page, start_time_ref, timeout_sec=5.0, cancel_check_fn=cancel_check_fn)
            ...
            for idx in range(2, total_steps):
                ...
                try:
                    ...
                    if stype == "cc_map":
                        ...
                        clicked_list = _click_command_center_list_view(page, list_btn)
                        if not clicked_list:
                        ...
                        load_time, rec_count = _wait_for_command_center_list_data(
                            page,
                            start_list,
                            {},
...
def _delete_generated_ui_report_safely(page, stype, param):
    if stype not in ("report_alert", "report_other"):
        return
    print(f"[DOWNLOAD WORKER] Cleaning up generated report record on UI for step {param}...", flush=True)
    try:
        ...
        if first_row.count() > 0 and first_row.is_visible():
            action_menu_selectors = [
                ...
                'button:has(mat-icon:has-text("more"))',
            ...
            menu_clicked = False
            for m_sel in action_menu_selectors:
                menu_btn = first_row.locator(m_sel).first
                if menu_btn.count() > 0 and menu_btn.is_visible() and menu_btn.is_enabled():
                    print(f"[DOWNLOAD WORKER] Found action menu button: {m_sel}. Clicking it to reveal options...", flush=True)
                    menu_btn.click()
                    page.wait_for_timeout(1000)
                    menu_clicked = True
                    break
            ...
            delete_selectors = [
                ...
                'i.material-icons:has-text("delete")',
                'i.fa-trash',
                '.delete-btn',
                '.remove-btn',
                '[class*="delete"]',
                ...
                '[role="menuitem"]:has-text("deleteDelete")',
            ...
            if menu_clicked:
                for sel in delete_selectors:
                    btn = page.locator(f".mat-menu-panel {sel}, .dropdown-menu {sel}, .mat-menu-content {sel}, {sel}").first
                    if btn.count() > 0 and btn.is_visible() and btn.is_enabled():
                        print(f"[DOWNLOAD WORKER] Clicking delete option in revealed menu: {sel}", flush=True)
                        btn.click()
                        page.wait_for_timeout(1000)
                        deleted = True
                        break
            ...
            if not deleted:
                for sel in delete_selectors:
                    btn = first_row.locator(sel).first
                    if btn.count() > 0 and btn.is_visible() and btn.is_enabled():
                        print(f"[DOWNLOAD WORKER] Clicking direct delete button in row: {sel}", flush=True)
                        btn.click()
                        page.wait_for_timeout(1000)
                        deleted = True
                        break
            ...
            if deleted:
                ...
                confirm_selectors = [
                    ...
                    'button.btn-danger:has-text("Yes")'
                ]
                for c_sel in confirm_selectors:
                    confirm_btn = page.locator(c_sel).first
                    if confirm_btn.count() > 0 and confirm_btn.is_visible() and confirm_btn.is_enabled():
                        print(f"[DOWNLOAD WORKER] Confirming delete in popup: {c_sel}", flush=True)
                        confirm_btn.click()
                        page.wait_for_timeout(2000)
                        print(f"[DOWNLOAD WORKER] Cleanup successful for step {param}!", flush=True)
                        break
            else:
                print(f"[DOWNLOAD WORKER] No delete button or option found for step {param}.", flush=True)
                
    except Exception as cleanup_err:
        print(f"[DOWNLOAD WORKER] Cleanup of page report record failed: {cleanup_err}", flush=True)

def _download_kpi_runner_worker(app, site_id, run_id, jhs_credentials=None):
    """Background Playwright worker to gather KPI download sheet performance data."""
    print(f"\n[DOWNLOAD KPI RUNNER] Starting background crawl for run: {run_id}", flush=True)
    
    with app.app_context():
        site = Site.query.get(site_id)
        tr = TestRun.query.get(run_id)
        if not site or not tr:
            print("[DOWNLOAD KPI RUNNER] Target site or test run record not found.", flush=True)
            return
    ...
    def check_cancel():
        with _active_download_kpi_runs_lock:
            if run_id in _active_download_kpi_runs and _active_download_kpi_runs[run_id].get("status") == "cancelled":
                raise Exception("cancelled")

    total_steps = len(KPI_DOWNLOAD_STEPS)
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                ...
                print(f"[DOWNLOAD KPI RUNNER] Loading JHS storage state from shared path {state_file}...", flush=True)
                context = browser.new_context(storage_state=state_file)
                from app.utils.playwright_session import inject_session_storage_to_context
                inject_session_storage_to_context(context, site.name, email=username)
            except Exception as e:
                print(f"[DOWNLOAD KPI RUNNER] Error using shared session: {e}. Starting fresh context.", flush=True)
                context = browser.new_context()
            page = context.new_page()
            ...
            already_logged_in = False
            try:
                # Wait longer for the page to load and check multiple indicators of a logged-in state
                page.wait_for_selector(
                    ".sidebar-link, a[href*='dashboard'], .main-header, .nav-link, "
                    "button.allButton, .navbar-brand, nb-layout-column, .page-inner-content",
                    timeout=15000
                )
                already_logged_in = True
                print("[DOWNLOAD KPI RUNNER] Session is valid - already logged in.", flush=True)
            except Exception:
                print("[DOWNLOAD KPI RUNNER] Session check failed - not logged in.", flush=True)
                
            # If not logged in, force navigation back to login page
            if not already_logged_in:
                ...
                # Save storage state on manual login
                try:
                    page.wait_for_timeout(4000)
                    from app.utils.playwright_session import get_shared_session_path
                    state_file = get_shared_session_path(site.name)
            ...
            # Subsequent steps (index 0 onwards)
            for idx in range(0, total_steps):
                check_cancel()
                step = KPI_DOWNLOAD_STEPS[idx]
                param = step["parameter"]
                cat = step["category"]
                stype = step["type"]
                url = base_url + step["url_suffix"]
                ...
                try:
                    try:
                        ...
                        if stype == "cc_list":
                            ...
                            # Set up API response interception BEFORE clicking List View.
                            # Only Command Centre list APIs are accepted here; generic lookup
                            # APIs can legitimately return one row and must not end the wait.
                            api_response_data = {"load_time": None, "rec_count": None, "source_url": None}
                            
                            def on_response(response):
                                """Capture the vehicle list API response time and record count."""
                                try:
                                    ...
                                    if "json" not in content_type and "application/json" not in content_type:
                                        return
                                    body = response.json()
                                    total = _extract_command_center_list_api_total(response.url, body)
                                    if total and total > 0:
                                        api_response_data["rec_count"] = total
                                        api_response_data["load_time"] = time.time()
                                        api_response_data["source_url"] = response.url
                                        print(f"[DOWNLOAD WORKER] Command Centre list API captured: url={response.url[:120]}, records={total}", flush=True)
                                except Exception:
                                    pass
                            ...
                            source_url = api_response_data.get("source_url")
                            if source_url:
                                print(f"[DOWNLOAD WORKER] API/paginator-based: load_time={load_time}s, rec_count={rec_count}", flush=True)
                            else:
                                print(f"[DOWNLOAD WORKER] Paginator-based: load_time={load_time}s, rec_count={rec_count}", flush=True)
                            
                            # Wait for the table to fully render before clicking download
                            try:
                                page.wait_for_selector('button:has(i.fa-download):visible', timeout=15000)
                                page.wait_for_timeout(1000)  # Allow table to settle
                            except Exception:
                                pass
                            
                        elif stype == "video_tab":
                            tab_name = step["tab_name"]
                            page.wait_for_selector('.mat-tab-label, .page-inner-content .nav-link', timeout=15000)
                            tab_el = None
                            for sel in [f'.mat-tab-label:has-text("{tab_name}")', f'.page-inner-content .nav-link:has-text("{tab_name}")', f'div.mat-tab-label >> text={tab_name}', f'button:has-text("{tab_name}")']:
                            ...
                            try:
                                page.wait_for_selector('.loading, .loader, [class*="spinner"], mat-progress-bar', state="hidden", timeout=10000)
                            except Exception:
                                pass
                            ...
                            while time.time() - start_wait < 25.0:
                                try:
                                    page_text = page.evaluate("() => document.body.innerText")
                                    limit_patterns = ["more than 2000", "exceeds 2000", "maximum 2000", "2000 records"]
                                    if any(pat in page_text.lower() for pat in limit_patterns):
                                        rec_val = "2000+"
                                        break
                                except Exception:
                                    pass
                            ...
                            if veh_drop.count() > 0:
                                try:
                                    ...
                                    if dropdown_list.count() > 0 and dropdown_list.first.is_visible():
                                        print("[DOWNLOAD WORKER] Dropdown still open. Clicking body to force close...", flush=True)
                                        page.locator("body").click()
                                        page.wait_for_timeout(500)
                                except Exception:
                                    pass
                            ...
                            if choose_btn.count() > 0:
                                try:
                                    ...
                                    page.evaluate("""() => {
                                        const checkedCheckboxes = document.querySelectorAll('.Alert-dropdown input[type="checkbox"]:checked');
                                        checkedCheckboxes.forEach(cb => { cb.click(); cb.dispatchEvent(new Event('change', { bubbles: true })); });
                                    }""")
                                    page.wait_for_timeout(500)
                                            ...
                                            if lbl.is_visible():
                                                ...
                                                if should_check:
                                                    ...
                                                    if chk_input.count() > 0:
                                                        if chk_input.is_checked():
                                                            lbl.click()
                                    close_btn = page.locator('.Alert-dropdown span:has-text("close"), button:has-text("Close")').first
                                    if close_btn.count() > 0:
                                        close_btn.click()
                                        page.wait_for_timeout(500)
                                except Exception:
                            ...
                            start_rep = time.time()
                            start_action = start_rep
                            view_btn = page.locator('button[title="View"], button:has-text("View"), button:has-text("Submit")').first
                            view_btn.click(force=True)
                            try:
                                page.wait_for_selector('.loading, .loader, mat-progress-bar', state="hidden", timeout=30000)
                            except Exception:
                                pass
                            load_time, rec_count = _wait_and_capture_kpi_data(page, start_rep, cancel_check_fn=check_cancel)
                        ...
                        # Trigger download, measure expected kpi time, and immediately delete download from disk
                        try:
                            selectors = [
                                'button:has(i.fa-download)',
                                'button.download-btn:not(.form_btn)',
                                ...
                                'a:has-text("Download")',
                                'a:has-text("Export")',
                                'button:has(.fa-file-excel)',
                                'button:has(mat-icon:has-text("download"))',
                                '[title*="Download"]:not([title="Manage Columns"])',
                                '[title*="Export"]',
                                '.download',
                                '.export',
                                'button:has-text("CSV")',
                            ...
                            if stype == "cargo_tab":
                                container_prefix = f"#{tab_name.lower()}-view "
                                selectors = [container_prefix + sel for sel in selectors] + selectors
                                
                            clicked = False
                            start_click_wait = time.time()
                            with page.expect_download(timeout=60000) as download_info:
                                while time.time() - start_click_wait < 15.0:
                                    for sel in selectors:
                                        try:
                                            locators = page.locator(sel).all()
                                            for loc in locators:
                                                if loc.is_visible() and loc.is_enabled():
                                                    print(f"[DOWNLOAD WORKER] Clicking download button: {sel}", flush=True)
                                                    loc.click()
                                                    clicked = True
                                                    break
                                            if clicked:
                                    ...
                                    try:
                                        ...
                                        for btn in buttons:
                                            if btn.is_visible() and btn.is_enabled():
                                                html_val = btn.evaluate("el => el.outerHTML").lower()
                                                title_val = (btn.get_attribute("title") or "").lower()
                                                text_val = (btn.text_content() or "").lower()
                                                # Skip Manage Columns / form_btn buttons
                                                if "manage" in title_val or "form_btn" in html_val:
                                                    continue
                                                if any(x in html_val or x in title_val or x in text_val for x in ["download", "export", "save_alt", "excel", "csv", "fa-download"]):
                                                    print(f"[DOWNLOAD WORKER] Clicking fallback download: {html_val[:100]}", flush=True)
                                                    btn.click()
                                                    clicked = True
                                                    break
                                    except Exception:
                                ...
                                if not clicked:
                                    raise Exception("No active/enabled download button found within 15 seconds")
                                    
                            download = download_info.value
                            temp_path = download.path()
                            print(f"[DOWNLOAD WORKER] Download completed! Path: {temp_path}", flush=True)
                            download.delete()
                            print("[DOWNLOAD WORKER] Deleted downloaded temporary file successfully.", flush=True)
                            expected_kpi = f"{time.time() - start_action:.2f}"
                        except Exception as download_err:
                            print(f"[DOWNLOAD WORKER] Download process failed for step {param}: {download_err}", flush=True)
                            expected_kpi = "-"
                            
                    except Exception as step_err:
                        import traceback
                        tb_str = traceback.format_exc()
                        print(f"[DOWNLOAD KPI RUNNER] Error on step {param}: {step_err}\n{tb_str}", flush=True)
                        
                    finally:
                        _delete_generated_ui_report_safely(page, stype, param)
                        
                except Exception as fatal_step_err:
                    print(f"[DOWNLOAD KPI RUNNER] Fatal exception during step setup or cleanup: {fatal_step_err}", flush=True)
                    
                # Save captured details
                save_metric(f"{param} - Loading Time", cat, step["url_suffix"], load_time)
                save_metric(f"{param} - Number of Records", cat, step["url_suffix"], rec_count)
                save_metric(f"{param} - Expected KPI", cat, step["url_suffix"], expected_kpi)
                
                with _active_download_kpi_runs_lock:
                    if run_id in _active_download_kpi_runs:
                        _active_download_kpi_runs[run_id]["completed_steps"] += 1
        ...
        with _active_download_kpi_runs_lock:
            if run_id in _active_download_kpi_runs:
                if is_cancelled:
                    _active_download_kpi_runs[run_id]["status"] = "cancelled"
                else:
                    _active_download_kpi_runs[run_id]["status"] = "completed"
        ...
        with _active_download_kpi_runs_lock:
            if run_id in _active_download_kpi_runs and _active_download_kpi_runs[run_id].get("status") == "cancelled":
                is_cancelled = True
        print(f"[DOWNLOAD KPI RUNNER] Fatal exception: {exc}", flush=True)
        with app.app_context():
            tr = TestRun.query.get(run_id)
            if tr:
                tr.status = "cancelled" if (is_cancelled or str(exc) == "cancelled") else "error"
        ...
        with _active_download_kpi_runs_lock:
            if run_id in _active_download_kpi_runs:
                if is_cancelled or str(exc) == "cancelled":
                    ...
                    _active_download_kpi_runs[run_id]["status"] = "error"
                    _active_download_kpi_runs[run_id]["error"] = str(exc)

@kpi_runner_bp.route("/api/kpi/download-sheet/run", methods=["POST"])
def api_download_kpi_run():
    """Trigger a Download KPI crawl in a background thread."""
    ...
    tc = TestCase.query.filter_by(category="download_kpi").first()
    if not tc:
        tc = TestCase(
            name="Download KPI Sheet Run",
            situation_description="Runs download performance metrics collection across pages.",
            category="download_kpi",
            steps=[]
        )
        db.session.add(tc)
        db.session.commit()
    ...
    with _active_download_kpi_runs_lock:
        _active_download_kpi_runs[run_id] = {
            "status": "running",
            "completed_steps": 0,
            "total_steps": len(KPI_DOWNLOAD_STEPS),
            "site_name": site.name
    ...
    thread = threading.Thread(target=_download_kpi_runner_worker, args=(app, site.id, run_id, jhs_creds))
    thread.daemon = True
    thread.start()
    
    return jsonify({"run_id": run_id})

@kpi_runner_bp.route("/api/kpi/download-sheet/progress/<run_id>")
def api_download_kpi_progress(run_id):
    """Retrieve live status and values captured so far for a download run."""
    with _active_download_kpi_runs_lock:
        status_info = _active_download_kpi_runs.get(run_id)
    ...
    if status_info:
        ...
        if tr:
            return jsonify({
                ...
                "completed_steps": len(KPI_DOWNLOAD_STEPS),
                "total_steps": len(KPI_DOWNLOAD_STEPS),
                "metrics": metrics
            })
        return jsonify({"error": "Run not found"}), 404

@kpi_runner_bp.route("/api/kpi/download-sheet/cancel/<run_id>", methods=["POST"])
def api_download_kpi_cancel(run_id):
    """Cancel an active download KPI generation run."""
    with _active_download_kpi_runs_lock:
        if run_id in _active_download_kpi_runs:
            _active_download_kpi_runs[run_id]["status"] = "cancelled"
...
@kpi_runner_bp.route("/api/kpi/download-sheet/details/<run_id>")
def api_download_kpi_details(run_id):
    """Retrieve detailed KPI values for download run preview."""
    captures = ValueCapture.query.filter_by(run_id=run_id).all()
    metrics = {cap.label: cap.captured_value for cap in captures}
    
    kpi_rows = []
    for step in KPI_DOWNLOAD_STEPS:
        param = step["parameter"]
        cat = step["category"]
        lt_key = f"{param} - Loading Time"
        rc_key = f"{param} - Number of Records"
        ek_key = f"{param} - Expected KPI"
        kpi_rows.append({
            "category": cat,
            "parameter": param,
            "loading_time": metrics.get(lt_key, ""),
            "record_count": metrics.get(rc_key, ""),
            "expected_kpi": metrics.get(ek_key, "")
        })
...
@kpi_runner_bp.route("/api/kpi/download-sheet/download/<run_id>")
def api_download_kpi_download(run_id):
    """Generate and return styled Excel report for a download run."""
    captures = ValueCapture.query.filter_by(run_id=run_id).all()
    metrics = {cap.label: cap.captured_value for cap in captures}
    ...
    output = io.BytesIO()
    generate_download_kpi_excel(kpi_rows, output)
    output.seek(0)
    ...
    filename = f"{date_str}-{site_name.lower()}-{username}-download-kpi.xlsx"
    ...
    try:
        ...
        print(f"[DOWNLOAD KPI DOWNLOAD] Failed to record download in history: {e}", flush=True)
...
def api_kpi_details(run_id):
    ...
    is_download = False
    if tr.test_case and tr.test_case.category == "download_kpi":
        is_download = True
    ...
    kpi_rows = []
    steps = KPI_DOWNLOAD_STEPS if is_download else KPI_STEPS
    for step in steps:
        param = step["parameter"]
        cat = step["category"]
        lt_key = f"{param} - Loading Time"
        ...
        if is_download:
            ek_key = f"{param} - Expected KPI"
            row_data["expected_kpi"] = metrics.get(ek_key, "")
        kpi_rows.append(row_data)
        
    site_name = tr.site.name if tr.site else "jhs"
    started_at = tr.started_at.strftime('%b %d, %Y %I:%M:%S %p') if tr.started_at else "N/A"
